...
Se estiver a extrair apenas uma configuração parcial de um servidor de solicitação individuais, o documento de configuração pode ter qualquer nome. Se estiver a extrair mais de uma configuração parcial de um servidor de solicitação, o documento de configuração pode ter um nome a `<ConfigurationName>.mof`, onde _ConfigurationName_ é o nome da configuração da parcial, ou `<ConfigurationName>.<NodeName>.mof`, em que  _ConfigurationName_ é o nome da configuração da parcial, e _NodeName_ é o nome do nó de destino. Isto permite-lhe solicitação configurações do servidor de solicitação do Automation DSC do Azure.
