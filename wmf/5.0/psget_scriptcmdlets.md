...
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="53ff5-102">Cmdlets de PowerShellGet para gestão de Script</span><span class="sxs-lookup"><span data-stu-id="53ff5-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="53ff5-103">Script de encontrar o cmdlet</span><span class="sxs-lookup"><span data-stu-id="53ff5-103">Find-Script cmdlet</span></span>
<span data-ttu-id="53ff5-104">Script de encontrar o cmdlet permite-lhe detetar os ficheiros de script com os critérios de pesquisa diferentes, como o nome, etiqueta, filtro, nome do comando, intervalo de versão, versão exata, todas as versões, incluindo as respetivas dependências e repositórios de todos os registado ou específicos.</span><span class="sxs-lookup"><span data-stu-id="53ff5-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="53ff5-105">Exemplo de utilização:</span><span class="sxs-lookup"><span data-stu-id="53ff5-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="53ff5-106">Cmdlet de Script de guardar</span><span class="sxs-lookup"><span data-stu-id="53ff5-106">Save-Script cmdlet</span></span>
<span data-ttu-id="53ff5-107">Script de guardar cmdlet permite-lhe para rever o ficheiro de script ao guardá-lo numa localização especificada.</span><span class="sxs-lookup"><span data-stu-id="53ff5-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="53ff5-108">Cmdlets de Script de instalação e Get-InstalledScript</span><span class="sxs-lookup"><span data-stu-id="53ff5-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="53ff5-109">Cmdlet de Script de instalação permite-lhe instalar um ficheiro de script específico, juntamente com as respetivas dependências para o âmbito especificado.</span><span class="sxs-lookup"><span data-stu-id="53ff5-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="53ff5-110">Por predefinição, os scripts estão instalados para o âmbito de AllUsers.</span><span class="sxs-lookup"><span data-stu-id="53ff5-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="53ff5-111">Cmdlet Get-InstalledScript permite-lhe obter a lista de ficheiros de script que foram instalados utilizando o cmdlet de Script de instalação.</span><span class="sxs-lookup"><span data-stu-id="53ff5-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="53ff5-112">Nota de utilização: para permitir a gestão e localizar de scripts, depois de serem instalados, script de instalação irá crie uma pasta predefinida para armazenar os scripts em $home\Documents\WindowsPowerShell\Scripts e adicionar nessa pasta ao seu ambiente de caminho.</span><span class="sxs-lookup"><span data-stu-id="53ff5-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="53ff5-113">Se modificar o caminho for uma preocupação, utilize o Script de guardar em vez de Script de instalação.</span><span class="sxs-lookup"><span data-stu-id="53ff5-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="53ff5-114">Get-InstalledScripts e o Script de desinstalação só podem ser utilizado com scripts colocados no sistema através do Script de instalação.</span><span class="sxs-lookup"><span data-stu-id="53ff5-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="53ff5-115">Também pode utilizar o comando Get-nome &lt;InstalledScriptFileName&gt; obtê-lo.</span><span class="sxs-lookup"><span data-stu-id="53ff5-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="53ff5-116">Instalar duas localizações são adicionadas à variável de ambiente PATH na primeira utilização de um âmbito especificado.</span><span class="sxs-lookup"><span data-stu-id="53ff5-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="53ff5-117">Cmdlet de Script de atualização</span><span class="sxs-lookup"><span data-stu-id="53ff5-117">Update-Script cmdlet</span></span>
<span data-ttu-id="53ff5-118">Script de atualização cmdlet permite-lhe no local a atualização dos ficheiros de script que foram instalados utilizando o cmdlet de Script de instalação.</span><span class="sxs-lookup"><span data-stu-id="53ff5-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="53ff5-119">Cmdlet de Script de desinstalação</span><span class="sxs-lookup"><span data-stu-id="53ff5-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="53ff5-120">Cmdlets novos ScriptFileInfo e ScriptFileInfo de teste</span><span class="sxs-lookup"><span data-stu-id="53ff5-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="53ff5-121">Novo ScriptFileInfo cmdlet permite-lhe criar um novo ficheiro de script com metadados como versão, Guid, autor e a descrição, etc. Cmdlet de teste ScriptFileInfo permite-lhe para validar e obter os metadados do ficheiro de script.</span><span class="sxs-lookup"><span data-stu-id="53ff5-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="53ff5-122">Cmdlet Update-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="53ff5-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="53ff5-123">Cmdlet Update-ScriptFileInfo permite-lhe para atualizar os metadados do ficheiro de script existente.</span><span class="sxs-lookup"><span data-stu-id="53ff5-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="53ff5-124">Cmdlets de registo PSRepository e Set-PSRepository com script de suporte de partilha</span><span class="sxs-lookup"><span data-stu-id="53ff5-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="53ff5-125">Utilizar cmdlets de registo-PSRepository/Set-PSRepository para adicionar o **ScriptSourceLocation** e **ScriptPublishLocation** para o PSRepository.</span><span class="sxs-lookup"><span data-stu-id="53ff5-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="53ff5-126">Cmdlet de Script publicar</span><span class="sxs-lookup"><span data-stu-id="53ff5-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="53ff5-127">Script publicar cmdlet permite-lhe para publicar o ficheiro de script com metadados válido como versão, Guid, autor e a descrição, etc.</span><span class="sxs-lookup"><span data-stu-id="53ff5-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell
