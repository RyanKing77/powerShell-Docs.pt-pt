...
Um novo fluxo de informações estruturado agora pode ser utilizado para transmitir dados estruturados entre um script e seu host. **Write-Host** também foi atualizado para emitir sua saída no fluxo de informações, onde pode agora capturar ou silence-lo. A nova **escreva informações** cmdlet (juntamente com uma API WriteInformation correspondente) utilizado com **- InformationVariable** e **- InformationAction** parâmetros comuns permite uma maior flexibilidade e capacidade.
