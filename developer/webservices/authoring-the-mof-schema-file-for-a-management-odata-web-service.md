...
---
# <a name="authoring-the-mof-schema-file-for-a-management-odata-web-service"></a>Authoring the MOF schema file for a Management OData web service (Autorizar o ficheiro de esquema MOF para um serviço Web OData de Gestão)

Definir os recursos que o serviço da web OData da gestão expõe através da criação de um ficheiro MOF que utilizado o esquema de recurso de público. Cada recurso é definido como uma classe no arquivo e as propriedades são definidas como membros de classe. Para obter mais informações sobre o esquema utilizado no ficheiro MOF, consulte [público de recurso de esquema](./public-resource-schema.md).

## <a name="example-mof-file"></a>Exemplo de ficheiro MOF

O arquivo a seguir define os recursos de serviço e o processo. Cada um destes recursos corresponde a um objeto que pode ser gerido por um conjunto de cmdlet do Windows PowerShell. As propriedades correspondem aos parâmetros utilizados por esses cmdlets.

Cada um dos dois recursos contém as propriedades de tipo complexo. Os tipos complexos são definidos como classes modificadas com o `ComplexType` qualificador.
