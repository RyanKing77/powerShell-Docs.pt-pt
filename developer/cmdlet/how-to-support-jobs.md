...
---
# <a name="how-to-support-jobs"></a>How to Support Jobs (Como Suportar Tarefas)

Este exemplo mostra como suportar tarefas quando escreve cmdlets. Se pretender que os usuários executem seu cmdlet como uma tarefa em segundo plano, tem de incluir o código descrito no procedimento seguinte. Para obter mais informações sobre tarefas em segundo plano, consulte [tarefas em segundo plano](./background-jobs.md).

## <a name="to-support-jobs"></a>Para suportar tarefas

1. Definir um `AsJob` mudar o parâmetro para que o usuário pode decidir se pretende executar o cmdlet como uma tarefa.

    O exemplo seguinte mostra uma declaração de parâmetro AsJob.
...
2. Criar um objeto que deriva de [System.Management.Automation.Job](/dotnet/api/System.Management.Automation.Job) classe. Este objeto pode ser um objeto de tarefa personalizada ou um dos objetos de trabalho fornecidos pelo Windows PowerShell, tal um [System.Management.Automation.Pseventjob](/dotnet/api/System.Management.Automation.PSEventJob) objeto.

    O exemplo seguinte mostra um objeto de tarefa personalizada.
...
3. Num método de processamento de registo, adicione um `if` instrução para detetar se o cmdlet deve ser executado como uma tarefa. O seguinte código utiliza a [System.Management.Automation.Cmdlet.ProcessRecord](/dotnet/api/System.Management.Automation.Cmdlet.ProcessRecord) método.
...
4. Para objetos de trabalho personalizados, implemente a classe de tarefa.
...
5. Se o cmdlet realiza o trabalho, chamar o [System.Management.Automation.Cmdlet.WriteObject](/dotnet/api/System.Management.Automation.Cmdlet.WriteObject) método para retornar um objeto de processo para o pipeline. Se o trabalho é executado como um trabalho, adicione tarefa subordinada à tarefa.
...
O código de exemplo seguinte mostra o código para um **Get-Proc** cmdlet que pode obter os processos internamente ou utilizando uma tarefa em segundo plano.
