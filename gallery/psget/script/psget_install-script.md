...
O cmdlet de Script de instalação adquire um payload de script de um repositório, verifica se o payload é um script do PowerShell válido e copia o ficheiro de script para uma localização de instalação especificada.

Repositórios de predefinido do que script de instalação funciona contra são configuráveis através do registo PSRepository, PSRepository conjunto, Unregister-PSRepository e cmdlets Get-PSRepository. Durante o funcionamento contra vários repositórios, o Script de instalação instala o script primeiro que satisfaça os critérios de pesquisa especificada (nome, MinimumVersion ou MaximumVersion) do repositório primeiro sem quaisquer erros.


Cmdlet de Script de instalação transfere um ou mais módulos da Galeria online, valida e instala-os no computador local para o âmbito de instalação especificada.

O cmdlet de Script de instalação verifica se os resultados da pesquisa são módulos válidos e copia pastas de módulo para a localização de instalação, recebe um ou mais módulos que cumprem critérios específicos de uma galeria online.
...
- Nenhum suporte para a versão do lado a lado para ficheiros de Script do PowerShell
- Suporte de instalação de dependência de scripts
...
Carateres universais não são suportadas em - nome do módulo de instalação, guardar-Module, desinstale-Module, Script de instalação, guardar scripts e cmdlets do Script de desinstalação.
...
Se o valor especificado para o parâmetro de nome corresponde a uma versão do módulo no computador e não adicionou o parâmetro MinimumVersion ou RequiredVersion, o Script de instalação silenciosamente continua sem instalar esse módulo. Se os parâmetros MinimumVersion ou RequiredVersion são especificados e o módulo existente não correspondem aos valores que parâmetro, em seguida, ocorre um erro. Para ser mais específico: se a versão do módulo atualmente instalada é inferior ao valor do parâmetro MinimumVersion ou não é igual ao valor do parâmetro RequiredVersion, ocorre um erro. Se a versão do módulo instalado é igual ao valor do parâmetro RequiredVersion ou maior do que o valor do parâmetro MinimumVersion, o Script de instalação silenciosamente continua sem instalar esse módulo.

Script de instalação devolve um erro de não existir nenhum módulo na Galeria online que corresponde ao nome especificado.
...
[Script de instalação](http://go.microsoft.com/fwlink/?LinkId=619784)
...
PowerShellGet suporta o suporte da versão lado a lado (SxS) módulo no Script de instalação, Script de atualização e publicar Script cmdlets que são executados no Windows PowerShell 5.0 ou mais recente.

### <a name="install-script-examples"></a>Exemplos de Script de instalação
...
## <a name="installing-a-script-with-dependent-scripts-and-modules"></a>A instalação de um script com dependentes scripts e de módulos
