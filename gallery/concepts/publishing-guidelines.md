...
Este tópico descreve os passos recomendados utilizados por equipas da Microsoft para garantir que os pacotes publicados na galeria do PowerShell amplamente adotados e fornecem valor elevado aos utilizadores, com base na forma como a galeria do PowerShell processa dados manifesto e nos comentários de grande número de utilizadores de galeria do PowerShell.
Pacotes que são publicadas seguir estas diretrizes serão maior probabilidade de ser instalado, confiar e attract mais utilizadores.

Incluída abaixo, são as diretrizes para o que faz com que um pacote de galeria do PowerShell boa, as definições de manifesto opcionais são mais importantes, melhorando o seu código com comentários de revisores iniciais e [analisador de Script do Powershell](https://aka.ms/psscriptanalyzer), controlo de versões do módulo, documentação, testes e exemplos de como utilizar o que tenha partilhado.
...
Para mechanics de publicação de um pacote de galeria do PowerShell, consulte [criar e publicar um pacote](/powershell/gallery/how-to/publishing-packages/publishing-a-package).

Comentários sobre estas diretrizes é welcomed. Se tiver comentários, abra os problemas no nosso [repositório do Github documentação](https://github.com/powershell/powershell-docs/issues).
...
Os pacotes que siga estas diretrizes são muito maior probabilidade de ser transferidos e adotado uma larga maioria por outras pessoas.
...
- Etiqueta do pacote com as plataformas e PSEdition(s) compatível 
...
- Utilizar PowerShellGet para publicar
...
Todos os pacotes publicados na galeria do PowerShell serão analisados utilizando PSScriptAnalyzer e quaisquer erros serão comunicados novamente para o proprietário e têm de ser resolvidos.
...
Os utilizadores que adquirir pacotes da galeria do PowerShell são vivamente recomendados para executar PSScriptAnalyzer e avaliar todos os erros e avisos.
Os utilizadores são muito prováveis contactar os proprietários de pacote se de que veem que existe um erro comunicado pelo PSScriptAnalyzer.
Se existir uma razão apelativa do seu pacote para manter o código que está assinalado como um erro, adicione essa informação para a documentação para evitar a necessidade que responder a mesma pergunta demasiadas vezes.
...
Documentação é a coisa mais útil para incluir pacotes publicados na galeria do PowerShell.
Os utilizadores, geralmente, irão ignorar pacotes sem documentação, como a alternativa é ler o código para compreender o que é o pacote e como utilizá-la.
Existem vários artigos disponíveis sobre como fornecer documentação com os pacotes de PowerShell, incluindo:
...
- A criação de ajuda do cmdlet, que é a melhor abordagem para qualquer script do PowerShell, a função ou o cmdlet.
  Para obter informações sobre como criar a ajuda do cmdlet, começar a utilizar [como escrever ajuda do Cmdlet](https://go.microsoft.com/fwlink/?LinkID=123415).
  Para adicionar ajuda dentro de um script, consulte [sobre comentário com base em ajudar](/powershell/module/microsoft.powershell.core/about/about_comment_based_help).
...
Exemplos mostram os utilizadores como o pacote se destina a ser utilizado.
...
Os proprietários de pacote que respondem corretamente aos comentários são altamente valor pela Comunidade.
Os utilizadores que constructive comentários são importantes para responder a, conforme forem interessados no pacote para experimentar ajudar a melhorar esta.

Existem dois métodos de comentários disponíveis na galeria do PowerShell:

- Contacte o proprietário do: Isto permite que um utilizador enviar um e-mail para o pacote owner(s). Como proprietário do pacote, é importante para monitorizar o endereço de e-mail utilizado com os pacotes de galeria do PowerShell e responder a problemas que são gerados. A um desvantagem a este método é a que apenas o utilizador e o proprietário alguma vez Verão a comunicação, pelo que pode ter o proprietário para responder a mesma pergunta demasiadas vezes.
- Comentários: Na parte inferior do pacote de página é um campo de comentário.
  ...
  Como proprietário do pacote, recomenda-se vivamente que seguem os comentários efetuados para cada pacote.
Consulte [fornecer comentários através de redes sociais ou comentários](../how-to/working-with-packages/social-media-feedback.md) para obter detalhes sobre como fazê-lo.
...
A estrutura do módulo permite, incluindo os outros pacotes que lista como as melhores práticas: cmdlet ajuda, documentação, exemplos e testes.
...
Um Site de projeto é onde um publicador pode interagir diretamente com os utilizadores com os seus pacotes de galeria do PowerShell.
Os utilizadores preferem pacotes que fornecem, como permite-los obter informações sobre o pacote mais facilmente.
Número de pacotes na galeria do PowerShell é desenvolvida no GitHub, outros são fornecidos por organizações com uma presença na web dedicado.
...
Quando é fornecido um ProjectURI, Galeria do PowerShell irá incluir uma ligação para o Site de projeto no lado esquerdo da página de pacote.

## <a name="tag-your-package-with-the-compatible-pseditions-and-platforms"></a>Etiqueta do pacote com as plataformas e PSEdition(s) compatível 

Utilize as seguintes etiquetas para demonstrar a utilizadores que pacotes irão funcionar bem com o seu ambiente:
...
- Linux : Pacotes que são compatíveis com os sistemas operativos Linux 
- MacOS : Pacotes que são compatíveis com o sistema de operativo Mac
...
Todos os pacotes publicados na galeria do PowerShell tem de especificar os termos de licenciamento ou estar vinculados pela licença incluída no [os termos de utilização](https://www.powershellgallery.com/policies/Terms) em "A apresentem um".
...
Assinatura de código fornece aos utilizadores com o nível mais elevado de garantia de que publicou o pacote e que a cópia do código adquirir é exatamente o que o publicador lançada.
...
Detalhes sobre como assinar os ficheiros de script do PowerShell é abordada o [sobre assinatura](/powershell/module/microsoft.powershell.core/about/about_signing) tópico.
...
Pode ser restringido PowerShell utilizando o [política de execução](/powershell/module/microsoft.powershell.core/about/about_execution_policies) assinada de cmdlets para garantir a utilização de scripts.

Catálogo de módulos de assinatura é uma funcionalidade adicionada para o PowerShell na versão 5.1.
Como assinar um módulo é abordado o [catálogo Cmdlets](/powershell/wmf/5.1/catalog-cmdlets) tópico.
...
O módulo publicar PowerShellGet, install-module, guardar-module e os cmdlets do módulo de atualização irá verificar a assinatura para garantir que é válido, em seguida confirmar que o valor de hash para cada pacote corresponde ao que está no catálogo.
...
A versão do seu pacote tem de ser incluída nos dados de manifesto.

- A versão deve ser estruturada como 3 blocos numéricos separados por pontos, tal como em 0.1.1 ou 4.11.192
- Versões que iniciam com "0" indicam que o pacote ainda não está pronta de produção e o primeiro número só deve começar por "0", se o que é o número de apenas utilizado
- Alterações ao número primeiro (1.9.9999 para 2.0.0) indicam as alterações principais e de última hora entre as versões
- As alterações para o segundo número (1.1 para 1.2) indicam as alterações a nível de funcionalidade, como adicionar novos cmdlets para um módulo
...
Um ponto adicional sobre a publicação de teste: não é possível eliminar qualquer pacote publicar para a galeria do PowerShell da ajuda da equipa de operações, que confirma que nada está dependente do pacote que pretende publicar.
...
## <a name="use-powershellget-to-publish"></a>Utilizar PowerShellGet para publicar

É vivamente recomendado que editores de utilizam os cmdlets do módulo de publicar e publicar Script ao trabalhar com a galeria do PowerShell.
PowerShellGet foi criada para o ajudar a evitar a memorizar detalhes importantes sobre a instalação do e publicação de galeria do PowerShell.
No occasion, publicadores tem optado por ignorar PowerShellGet e utilizar o cliente do NuGet, PackageManagement cmdlets ou, em vez do módulo de publicar.
Existem vários detalhes que são facilmente perdida, que resulta numa variedade de pedidos de suporte.

Se existir um motivo que não é possível utilizar módulo de publicar ou Script publicar,. permitem-nos informar.
O ficheiro de um problema no repositório PowerShellGet GitHub e forneça os detalhes que fazer com que escolher NuGet ou PackageManagement.

## <a name="recommended-workflow"></a>Fluxo de trabalho recomendado

A abordagem mais bem-sucedida que encontrámos pacotes publicado na galeria do PowerShell é:
...
- Decida se pretende código assinar o pacote
